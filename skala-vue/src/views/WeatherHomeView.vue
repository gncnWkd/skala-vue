...
const handleSelectCity = (name) => {
  statusMessage.value = `${name}의 날씨 요약을 확인하고 있어요.`
}
  ...
  <div class="weather-home">
    <section class="hero">
      <div>
        ...
        <p class="hero-description">
          주요 도시의 현재 기상 정보를 빠르고 깔끔하게 확인할 수 있습니다.
        </p>
      </div>
      <el-button
    ...
    <section class="dashboard-section overview-grid">
      <BaseDashboardCard class="weather-map">
        <div class="map-copy">
          <p class="section-kicker">KOREA WEATHER</p>
          <h2>전국 날씨 관측</h2>
          <p>서울을 중심으로 실시간 데이터를 수집하고 있어요.</p>
          <br />
          <iframe
            width="600"
            height="400"
            src="https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=default&metricTemp=default&metricWind=default&zoom=7&overlay=temp&product=ecmwf&level=surface&lat=36.72&lon=127.752"
            frameborder="0"
          ></iframe>
        </div>
      </BaseDashboardCard>
      <aside class="tip-card">
        <div class="tip">
          <span>✦</span>
          <div>
            <strong>날씨 단위 변경</strong>
            <p>상단의 °C 버튼을 눌러 화씨로 바꿔볼 수 있어요.</p>
          </div>
        </div>
        <div class="tip">
          <span>✦</span>
          <div>
            <strong>기온 지도</strong>
            <p>전국 기온 지도를 살펴볼 수 있어요.</p>
          </div>
        </div>
        <div class="tip">
          <span>✦</span>
          <div>
            <strong>지역별 상세 날씨</strong>
            <p>날씨 카드의 상세보기를 누르면 자세한 날씨 정보를 볼 수 있어요.</p>
          </div>
        </div>
        <div class="tip">
          <span>✦</span>
    ...
    <section class="dashboard-section">
      <div class="section-heading">
        <div>
          <p class="section-kicker">LOCAL WEATHER</p>
          <h2>지역별 날씨</h2>
        </div>
        <p class="result-count">{{ filteredWeatherList.length }}개 도시</p>
      </div>
      <BaseDashboardCard>
        <el-skeleton
          v-if="isLoading && !weatherData.length"
          :rows="3"
          animated
          class="state-message"
        />
        <el-alert
          ...
          :closable="false"
          show-icon
          title="날씨 정보를 가져오지 못했습니다. 잠시 후 다시 시도해 주세요."
        />
        <el-empty
          v-else-if="!filteredWeatherList.length"
          class="state-message"
          :image-size="72"
...
.tip-card {
  ...
  border-radius: 20px;
  color: #e8efff;
  background: linear-gradient(140deg, #2658c4, #4d79f2);
