...
watch(
  () => route.params.cityId,
  (cityId) => weatherStore.fetchWeatherDetail(cityId),
  { immediate: true },
)
</script>
...
<template>
  <section class="detail-page">
    ...
    <el-result
      v-else-if="error || !cityDetail"
      class="state-card"
      icon="error"
      title="지역 정보를 불러오지 못했습니다"
      sub-title="잠시 후 다시 시도해 주세요."
    />
    <template v-else>
      <header class="detail-hero">
        <div>
          <p class="eyebrow">LOCAL WEATHER</p>
          <h1>{{ cityDetail.name }} &nbsp; <span>현재 날씨</span></h1>
          <p>{{ cityDetail.status }} · 실시간 기상 관측 정보</p>
        </div>
      ...
      <el-descriptions class="metrics-card" :column="3" border direction="vertical">
        <el-descriptions-item label="💧 습도"
          ><strong> {{ cityDetail.humidity }}<small>%</small></strong></el-descriptions-item
        >
        <el-descriptions-item label="〰 풍속"
          ><strong> {{ cityDetail.wind }}<small>m/s</small></strong></el-descriptions-item
        >
        <el-descriptions-item label="☁ 기상 현황"
          ><strong class="status-value"> {{ cityDetail.status }}</strong></el-descriptions-item
        >
      </el-descriptions>
...
.detail-hero {
  ...
  border-radius: 24px;
  color: #fff;
  background: linear-gradient(130deg, #174dbd, #2e7dff);
  box-shadow: 0 20px 42px rgba(29, 89, 204, 0.24);
}
.eyebrow {
  color: #b9d6ff;
  font-size: 11px;
...
.metrics-card {
  ...
  overflow: hidden;
  border-radius: 20px;
  box-shadow: 0 8px 22px rgba(27, 55, 100, 0.035);
}
.metrics-card :deep(.el-descriptions__label) {
  color: var(--muted);
  font-weight: 600;
}
.metrics-card :deep(.el-descriptions__content strong) {
  font-size: 25px;
...
@media (max-width: 640px) {
  ...
  .metrics-card :deep(.el-descriptions__table) {
    display: block;
  }
  .metrics-card :deep(tbody),
  .metrics-card :deep(tr),
  .metrics-card :deep(td) {
    display: block;
    width: 100%;
  }
  .metrics-card :deep(.el-descriptions__cell) {
    border-width: 0 0 1px !important;
  }
  .metrics-card :deep(tr:last-child .el-descriptions__cell) {
